...
let string_split sep source =
  ...
  let rec loop prev current acc =
    if current >= l then List.rev acc
    else
      match (source.[current] = sep, current = prev, current = l - 1) with
      | true, true, _ -> loop (current + 1) (current + 1) acc
      | true, _, _ ->
          loop (current + 1) (current + 1) (copy_part prev current :: acc)
      | false, _, true ->
          loop (current + 1) (current + 1) (copy_part prev (current + 1) :: acc)
...
let search_header header =
  let rec loop = function
    | [] -> None
    | dir :: dirs ->
        if Sys.file_exists (dir ^ "/include/" ^ header) then Some dir
        else loop dirs
  in
  loop search_paths
...
let c_args =
  let flags path =
    Printf.sprintf "-ccopt -I%s/include -ccopt -L%s/lib" path path
  in
  match search_header "ev.h" with None -> "" | Some path -> flags path

let compile c_args args stub_file =
  let cmd =
    sprintf "%s -custom %s %s %s %s > %s 2>&1" !ocamlc c_args
      (Filename.quote stub_file) args
      (Filename.quote !caml_file)
      (Filename.quote !log_file)
  in
  Sys.command cmd = 0

let safe_remove file_name = try Sys.remove file_name with exn -> ()
...
let test_code args stub_code =
  ...
  let cleanup () =
    ...
    safe_remove
      (Filename.chop_extension (Filename.basename stub_file) ^ !ext_obj)
  in
  try
...
let () =
  let args =
    [
      ...
      ( "-exec-name",
        Arg.Set_string exec_name,
        "<name> name of the executable produced by ocamlc" );
    ]
  in
  Arg.parse args ignore
    "check for external C libraries and available features\noptions are:";
  ...
  if missing <> [] then (
    printf
      ...
      \      The following recquired C libraries are missing: %s.\n\
       ...
       location, set the environment variables C_INCLUDE_PATH and LIBRARY_PATH \
